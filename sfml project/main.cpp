...
int main()
{
    ...
    RenderWindow app(VideoMode(706,689), "Game 1");
    app.setFramerateLimit(60);

    Texture t1,t2;
    t1.loadFromFile("images/shahmatka.png");
    t2.loadFromFile("images/animals.png");

    Sprite shahmatka(t1), animals(t2);
   ...
   if (!isMoving)
    ...
    for (int i=1;i<=8;i++)
     for (int j=1;j<=8;j++)
      {
        piece p = grid[i][j];
        animals.setTextureRect( IntRect(p.kind*42,0,42,42));
